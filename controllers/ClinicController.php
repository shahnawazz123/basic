...
class ClinicController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if (Yii::$app->request->isAjax && $model->load(Yii::$app->request->post())) {
            ...
            if (isset($request['Clinics']['location']) && $request['Clinics']['location'] != "") {
                $model->location = $request['Clinics']['location'];
            }
            $model->type = 'C';
    ...
    public function actionDelete($id)
    {
        ...
        $model->is_deleted = 1;
        $model->save(false);
        Yii::$app->session->setFlash('success', 'Clinics successfully deleted');
        return $this->redirect(['index']);
    }
    ...
    public function actionProfile()
    {
        if (Yii::$app->session['_eyadatAuth'] == 2) {
            $id = Yii::$app->user->identity->clinic_id;
        } else {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $model->updated_at = date('Y-m-d H:i:s');
            $model->type = 'C';
            if (isset($request['Clinics']['password']) && $request['Clinics']['password'] != "") {
                $model->password = Yii::$app->getSecurity()->generatePasswordHash($request['Clinics']['password']);
            }
