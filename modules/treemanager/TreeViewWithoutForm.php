 ...
 * @since  1.0
 */
class TreeViewWithoutForm extends Widget
{
...
HTML;
    ...
    protected function validateSourceData()
    {
        ...
        $trait = 'app\modules\treemanager\models\TreeTrait';
    ...
    protected function renderNodeIcon($icon, $iconType, $child = true)
    {
        ...
        //return Html::tag('span', $this->defaultChildNodeIcon, $this->childNodeIconOptions);
        return "";
    }
    ...
    public function renderWidget()
    {
        $content = strtr($this->mainTemplate, [
            ...
            '{product_list}' => $this->renderProductList(),
        ]);
    ...
    public function renderTree()
    {
        ...
        foreach ($this->_nodes as $node) {
            ...
            if (!empty($node)) {
                ...
                if (isset($request['id']) && $request['id'] != "") {
                    $productCategory = \app\models\ProductCategories::find()
                            ->where(['product_id' => $request['id'], 'category_id' => $node->category_id])
                            ->one();
            ...
            if ($this->showCheckbox && ($node->isSelected() || $checkboxChecked)) {
                $css[] = 'kv-selected ';
            }
            if ($node->isCollapsed()) {
                $css[] = 'kv-collapsed ';
            ...
            $out .= Html::beginTag('li', $nodeOptions) . "\n" .
                    ...
                    $indicators . "\n" .
                    '</div>' . "\n" .
                    Html::beginTag('div', ['tabindex' => -1, 'class' => 'kv-node-detail-no-click']) . "\n" .
    ...
    {
        $content = $this->render('@app/views/product/_product_list');
        return Html::tag('div', $content, $this->detailOptions);
    }
