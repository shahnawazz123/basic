...
class Generator extends \yii\gii\Generator
{
    ...
    public function generateActiveField($attribute) {
        ...
        if (!empty($allForeignKeys)) {
            foreach ($allForeignKeys as $afk) {
                if ($afk['column'] === $attribute) {
                    $attTableName = $afk['table'];
                    $attColumnName = $afk['column'];
                    $isFkKey = true;
                    break;
                }
            }
        }
        $attTableName = str_replace('tbl_', '', $attTableName);
        $attTableName = ucfirst($attTableName);
        ...
        if (is_array($column->enumValues) && count($column->enumValues) > 0) {
            ...
            return "\$form->field(\$model, '$attribute')->dropDownList("
                    . preg_replace("/\n\s*/", ' ', VarDumper::export($dropDownOptions)) . ", ['prompt' => 'Please Select', 'class' => 'form-control select2])";
        }

        if (!$isFkKey && ($column->phpType !== 'string' || $column->size === null)) {
            return "\$form->field(\$model, '$attribute')->$input()";
        }
        if ($isFkKey) {
            $foreignTable = '';
            $nameArr = explode('_', $attTableName);
            foreach ($nameArr as $ar){
                $foreignTable.= ucfirst($ar);
            }
            
            $class = '\app\models\\'.$foreignTable;
            $pk = $class::primaryKey();
            $modelClass = new $class();
            $modelAttributes = $modelClass->attributes();
            $nameColumn = $pk[0];
            $where = '';
            foreach ($modelAttributes as $att){
                if($att=='name' || $att=='name_en' || $att=='title' || $att=='title_en'){
                    $nameColumn = $att;
                }
                if($att=='is_deleted'){
                    $where = "->where(['$att' => 0])";
            ...
            $list = "$class::find(){$where}->orderBy(['$nameColumn' => SORT_DESC])->all()";
            $listData = 'ArrayHelper::map('.$list.',"'.$pk[0].'","'.$nameColumn.'")';
            return "\$form->field(\$model, '$attribute')->dropDownList($listData, ['prompt' => 'Please Select','class' => 'form-control select2'])";
        }
